...
export const getMonthReceipt = new Elysia()
  ...
  .get('/metrics/month-receipt', async ({ getCurrentUser }) => {
    ...
    const monthsReceipts = await db
      .select({
        monthWithYear: sql<string>`TO_CHAR(${orders.createdAt}, 'YYYY-MM')`, // Formatação da Data do Banco de dados
      ...
      .groupBy(sql`TO_CHAR(${orders.createdAt}, 'YYYY-MM')`) // Formatação da Data do Banco de dados
